...
class PartDetailsDialog(wx.Dialog):
    ...
    def __init__(self, parent, part):
        ...
        self.parent = parent
        self.part = part
        # Resolve window/scale/project context robustly
        self.window = wx.GetTopLevelParent(self) or self
        self.scale_factor = getattr(parent, "scale_factor", GetScaleFactor(self.window))
        project_path = getattr(parent, "project_path", os.environ.get("KIPRJMOD", str(PLUGIN_PATH)))
        self.datasheet_path = Path(project_path) / "datasheets"
